...
void MainWindow::on_comboBox_activated(const QString &arg1)
{
   ...
   if (arg1=="num_aff")
   {
    qry->prepare("Select * from AFFAIRES_JURIDIQUES order by NUM_AFF desc");
    qry->exec();
    modal->setQuery(*qry);
    ui->tableView->setModel(modal);
    ui->tableView->show();
   }
  else if (arg1=="num et type")
   {
    qry->prepare("Select * from AFFAIRES_JURIDIQUES order by NUM_AFF,TYPE_AFF");
    qry->exec();
    modal->setQuery(*qry);
    ui->tableView->setModel(modal);
    ui->tableView->show();
   ...
   else if (arg1=="num et type et date")
   {
    qry->prepare("Select * from AFFAIRES_JURIDIQUES order by NUM_AFF,TYPE_AFF,DATES desc");
    qry->exec();
    modal->setQuery(*qry);
    ui->tableView->setModel(modal);
    ui->tableView->show();
